...
const mockDataAdapter = {
    fetchInteractions: async (userId) => {
        if (userId === 'user123') {
            return [
                {
                    productId: '1',
    ...
    fetchFeatureValues: async (featureKey) => {
    ...
    fetchCandidateProducts: async ({ excludeIds }) => {
        const allProducts = [
            {
                _id: '3',
                price: 6000,
    ...
    fetchPopularProducts: async (limit) => [
        {
            _id: '5',
            price: 3000,
            name: 'Popular Item',
...
const engine = (0, src_1.default)(mockDataAdapter, {
    features: ['category', 'brand'],
    enableLogging: true,
    modelPath: './models/test-model', // Directory, not file
});
async function runTests() {
    console.log('=== Test 1: Normal Case (user123 with multiple interactions) ===');
    try {
